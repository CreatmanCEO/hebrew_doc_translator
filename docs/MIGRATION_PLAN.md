...
| P0-T08 | Каркас `LlmProvider` + `OpenRouterProvider` (только translate, без batch). **Trigger-point подключения `OPENROUTER_API_KEY`** — пользовательский ключ добавляется в env именно здесь, до этого шага вопрос не поднимаем. | M | P0-T05 | integration-тест с msw возвращает мок-перевод; ручной dry-run с реальным ключом возвращает перевод "שלום" → "hello"/"привет" |
